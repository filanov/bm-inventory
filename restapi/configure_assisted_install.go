...
type Config struct {
	...
	// AuthAgent Applies when the "X-Auth-Secret" header is set
	AuthAgent func(token string) (interface{}, error)

	// AuthUser Applies when the "X-Auth-Token" header is set
...
func HandlerAPI(c Config) (http.Handler, *operations.AssistedInstallAPI, error) {
	...
	api.AgentAuth = func(token string) (interface{}, error) {
		if c.AuthAgent == nil {
			return token, nil
		}
		return c.AuthAgent(token)
	}
