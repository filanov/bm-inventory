...
generate-from-swagger:
	...
	docker run -u $(UID):$(UID) -v $(PWD):$(PWD):rw,Z -v /etc/passwd:/etc/passwd -w $(PWD) quay.io/ocpmetal/go-swagger:latest  generate server --template=stratoscale -f swagger.yaml --template-dir=/templates/contrib
	docker run -u $(UID):$(UID) -v $(PWD):$(PWD):rw,Z -v /etc/passwd:/etc/passwd -w $(PWD) quay.io/ocpmetal/go-swagger:latest  generate client --template=stratoscale -f swagger.yaml --template-dir=/templates/contrib
