...
def load_deployment_options(parser=None):
    ...
    deploy_options.add_argument(
        "--deploy-tag", help="Tag for all deployment images", type=str
    )
    deploy_options.add_argument(
        "--deploy-manifest-tag",
        help="Tag of the assisted-installer-deployment repo to get the deployment images manifest from",
        type=str,
    )
    deploy_options.add_argument(
        "--deploy-manifest-path",
        help="Path to local deployment images manifest",
        type=str,
    )
...
def get_manifest_from_url(tag):
    manifest_file = get_file_content(
        "https://api.github.com/repos/eranco74/assisted-installer-deployment",
        tag,
        "assisted-installer.yaml",
    )
...
def get_image_override(deployment_options, short_image_name, env_var_name):
    ...
    if deployment_options.deploy_manifest_path:
        print(
            "Deploying {} according to manifest: {}".format(
                short_image_name, deployment_options.deploy_manifest_path
            )
        )
        ...
        print(
            "Deploying {} according to assisted-installer-deployment tag: {}".format(
                short_image_name, deployment_options.deploy_manifest_tag
            )
        )
        ...
        print(
            "Deploying {} with deploy tag {}".format(
                short_image_name, deployment_options.deploy_tag
            )
        )
    ...
    # In case non of the above options was used allow overriding specific images with env var
        ...
        print(
            "Overriding {} deployment image according to env {}".format(
                short_image_name, env_var_name
            )
        )
