...
def check_k8s_rollout(k8s_object, k8s_object_name, namespace="assisted-installer"):
    cmd = "{} rollout status {}/{} --namespace {}".format(
        "kubectl", k8s_object, k8s_object_name, namespace
    )
...
def check_if_exists(k8s_object, k8s_object_name, namespace="assisted-installer"):
    try:
        cmd = "{} get -n {} {} {} --no-headers".format(
            KUBECTL_CMD, namespace, k8s_object, k8s_object_name
        )
