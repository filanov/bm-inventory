...
def main():
    ...
    waiting.wait(
        lambda: is_pod_in_state(),
        timeout_seconds=TIMEOUT,
        expected_exceptions=Exception,
        sleep_seconds=1,
        waiting_for="pod app {} is in {} status".format(args.app, args.state),
    )
...
def get_pod_state():
    ret = subprocess.check_output(
        "kubectl get pods -l app={app} -o json --namespace=assisted-installer".format(
            app=args.app
        ),
        shell=True,
    )
