...
def deploy_oauth_reqs():
    ...
    dst_file = os.path.join(
        os.getcwd(), "build/assisted-installer-ocp-prometheus-custom-ca.yaml"
    )
    topic = "OCP Custom CA"
...
def deploy_prometheus_route():
    ...
    dst_file = os.path.join(
        os.getcwd(), "build/assisted-installer-ocp-prometheus-route.yaml"
    )
...
def deploy_prometheus_sub(olm_ns, cat_src):
    ...
    dst_file = os.path.join(
        os.getcwd(), "build/assisted-installer-operator-subscription.yaml"
    )
    with open(src_file, "r") as src:
        with open(dst_file, "w+") as dst:
            data = src.read()
...
def main():
    if args.target != "oc-ingress":
        ...
        utils.check_k8s_rollout(
            "statefulset", "prometheus-assisted-installer-prometheus"
        )
