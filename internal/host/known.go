...
func (k *knownState) UpdateHostname(ctx context.Context, h *models.Host, hostname string, db *gorm.DB) (*UpdateReply, error) {
	h.RequestedHostname = hostname
	cdb := k.db
	if db != nil {
		cdb = db
	}
	return updateHostname(logutil.FromContext(ctx, k.log), h, cdb)
}
