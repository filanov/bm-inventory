...
func NewHostStateMachine(th *transitionHandler) stateswitch.StateMachine {
	...
	var isSufficientForInstall = stateswitch.And(If(HasMemoryForRole), If(HasCPUCoresForRole), If(BelongsToMachineCidr), If(IsHostnameUnique), If(IsHostnameValid))
	...
	sm.AddTransition(stateswitch.TransitionRule{
		TransitionType:   TransitionTypeRefresh,
		SourceStates:     []stateswitch.State{HostStatusInstalled},
		DestinationState: HostStatusInstalled,
	})
	...
	sm.AddTransition(stateswitch.TransitionRule{
		TransitionType:   TransitionTypeRefresh,
		SourceStates:     []stateswitch.State{HostStatusInstalling},
		DestinationState: HostStatusInstalling,
	})

	sm.AddTransition(stateswitch.TransitionRule{
		TransitionType:   TransitionTypeRefresh,
		SourceStates:     []stateswitch.State{HostStatusResetting},
		DestinationState: HostStatusResetting,
	})
	sm.AddTransition(stateswitch.TransitionRule{
		TransitionType:   TransitionTypeRefresh,
		SourceStates:     []stateswitch.State{HostStatusInstallingPendingUserAction},
		DestinationState: HostStatusInstallingPendingUserAction,
	})
	...
	sm.AddTransition(stateswitch.TransitionRule{
		TransitionType:   TransitionTypeRefresh,
		SourceStates:     []stateswitch.State{HostStatusInstallingInProgress},
		DestinationState: HostStatusInstallingInProgress,
	})
