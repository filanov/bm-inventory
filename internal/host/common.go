...
func exists(db *gorm.DB, hostId, clusterId strfmt.UUID, where map[string]interface{}) bool {
...
func UpdateHost(log logrus.FieldLogger, db *gorm.DB, clusterId strfmt.UUID, hostId strfmt.UUID,
	...
	if dbReply.Error != nil || (dbReply.RowsAffected == 0 && !exists(db, hostId, clusterId, updates)) {
