...
func NewClusterStateMachine(th *transitionHandler) stateswitch.StateMachine {
	...
	var requiredInputFieldsExist = stateswitch.And(If(IsMachineCidrDefined), If(NoPendingForInputHost))
	var isSufficientForInstall = stateswitch.And(If(isMachineCidrEqualsToCalculatedCidr), If(isApiVipBelongToMachineCidrAndNotInUse),
		If(isIngressVipBelongToMachineCidrAndNotInUse), If(AllHostsAreReadyToInstall), If(HasExactlyThreeMasters))
	...
	sm.AddTransition(stateswitch.TransitionRule{
		...
		Condition:        stateswitch.Not(isSufficientForInstall),
		DestinationState: stateswitch.State(models.ClusterStatusInsufficient),
