...
var _ = Describe("installer", func() {
	...
	Context("install cluster", func() {
		...
		It("cluster is finalizing", func() {
			cluster = updateClusterState(cluster, clusterStatusFinalizing, db)
			err := installerManager.Install(ctx, &cluster, db)
			Expect(err.Error()).Should(MatchRegexp(errors.Errorf("cluster %s is already %s", cluster.ID, clusterStatusFinalizing).Error()))
		})
