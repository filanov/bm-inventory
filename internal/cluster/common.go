...
func updateClusterStatus(log logrus.FieldLogger, db *gorm.DB, clusterId strfmt.UUID, srcStatus string,
	newStatus string, statusInfo string) error {

	updates := []string{"status", newStatus, "status_info", statusInfo}
	now := strfmt.DateTime(time.Now()).String()

	if newStatus != srcStatus {
		updates = append(updates, "status_updated_at", now)
	}
	if srcStatus == clusterStatusReady && newStatus == clusterStatusPrepareForInstallation {
		updates = append(updates, "install_started_at", now)
	} else if srcStatus == clusterStatusInstalling && newStatus == clusterStatusInstalled {
		updates = append(updates, "install_completed_at", now)
	}

	extra := make([]interface{}, len(updates))
