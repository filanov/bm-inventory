...
var _ = Describe("cluster install", func() {
	...
	Context("install cluster cases", func() {
		...
		It("register host while installing only_k8s", func() {
			_, err := bmclient.Installer.InstallCluster(ctx, &installer.InstallClusterParams{ClusterID: clusterID})
			Expect(err).NotTo(HaveOccurred())
			rep, err := bmclient.Installer.GetCluster(ctx, &installer.GetClusterParams{ClusterID: clusterID})
			Expect(err).NotTo(HaveOccurred())
		...
		It("register host while cluster in error state only_k8s", func() {
		...
		It("register existing host while cluster in installing state only_k8s", func() {
		...
		It("install cluster only_k8s", func() {
			_, err := bmclient.Installer.InstallCluster(ctx, &installer.InstallClusterParams{ClusterID: clusterID})
			Expect(err).NotTo(HaveOccurred())
		...
		It("report_progress only_k8s", func() {
			c, err := bmclient.Installer.InstallCluster(ctx, &installer.InstallClusterParams{ClusterID: clusterID})
			Expect(err).NotTo(HaveOccurred())
		...
		It("install download_config_files only_k8s", func() {
		...
		It("download_config_files in error state only_k8s", func() {
			file, err := ioutil.TempFile("", "tmp")
			Expect(err).NotTo(HaveOccurred())
			defer os.Remove(file.Name())
		...
		It("Get credentials only_k8s", func() {
		...
		It("Upload ingress ca and kubeconfig download only_k8s", func() {
